...
suggest_movie(M) :-
    write('****WELCOME TO MOVIE RECOMMENDATION SYSTEM****\n The system will ask you a few questions an return a list of movies depending on your response. \n Otherwise it will return an empty list if no suitable recommendation can be made.\n\n\n '),
    write('Enter your age: '),
    read(Age),
    get_category(Age,Categories),
    write('Choose your mood: \n   happy   sad   angry   scared\n'),
    read(Mood),
    write('Choose your preferred movie director from the list below:
    \n
    john_hughes
    jeremy_kagun
    ridley_scott
    ...
    :\n'),
    read(Director),
    write('Choose your preferred streaming platform: \n
    netflix
    hbomax
    hulu
    prime
    disney:\n'),
    read(Stream),
    write('Enter minimum length of movie in minutes: '),
    read(Min),
    write('Enter maximum length of movie in minutes: '),
    read(Max),
...
get_theme(Mood,X) :-
    Mood = happy,
    X = [horror,comedy,action,sci-fi,animation,fantasy,war,musical,romance,thriller,crime,biography,family].

get_theme(Mood,X) :-
    Mood = sad,
    X = [comedy,family,mystery,drama,animation,fantasy,adventure,musical,romance].

get_theme(Mood,X) :-
    Mood = scared,
    X = [family,fantasy,musical,comedy,adventure,drama].

get_theme(Mood,X) :-
    Mood = angry,
    X = [comedy,family,sci-fi,animation,drama,musical,history].


%DATABASE
movie(american_graffiti,george_lucas,pg,110,[hbomax,netflix,prime],[comedy,drama]).
movie(sixteen_candles,john_hughes,pg,93,[prime,netflix,hulu],[comedy,romance]).
movie(empire_of_the_sun,steven_spielberg,pg,153,[hbomax,netflix,prime],[drama,history,war]).
movie(heavenly_creatures,peter_jackson,r,99,[prime,netflix],[biography,crime,drama]).
movie(the_journey_of_natty_gann,jeremy_kagun,pg,101,[prime,disney,netflix],[adventure,family]).
movie(ferris_buellers_day_off,john_hughes,pg13,103,[prime,netflix],[comedy]).
...
movie(justice_league,zack_snyder,pg13,120,[hbomax],[action,adventure,fantasy,sci-fi]).
...
movie(what_we_do_in_the_shadows,taika_waititi,r,86,[hbomax],[comedy,horror]).
movie(jojo_rabbit,taika_waititi,pg13,108,[hbomax],[comedy,drama,war]).
...
movie(last_tango_in_paris,bernardo_bertolucci,nc17,129,[hbomax],[drama,romance]).
movie(interstellar,christopher_nolan,pg13,169,[netflix,disney],[adventure,drama,sci-fi]).
movie(tenet,christopher_nolan,pg13,150,[netflix,hbomax],[action,thriller,sci-fi]).
...
movie(prestige,christopher_nolan,pg13,130,[hbomax],[drama,mystery,sci-fi]).
...
movie(the_godfather,drancis_ford_coppola,r,175,[hbomax],[drama,crime]).
...
movie(inglourious_basterds,quentin_tarantino,r,153,[hbomax],[drama,action,war]).
movie(a_beautiful_mind,ron_howard,pg13,135,[hbomax,prime],[drama]).
movie(the_incredibles,brad_bird,pg,115,[hbomax,disney],[animation,action,family,comedy,sci-fi]).
movie(the_incredibles_2,brad_bird,pg,118,[hbomax,disney],[animation,action,family,comedy,sci-fi]).
movie(titanic,james_cameron,pg13,194,[hbomax],[drama,romance]).
movie(skyfall,sam_mendes,pg13,143,[hbomax],[action]).
...
movie(finding_nemo,andrew_stanton,g,100,[hbomax],[animation,comedy,family]).
movie(zodiac,david_fincher,r,157,[hbomax,netflix],[crime,drama,mystery]).
movie(tangled,nathan_greno,pg,100,[hbomax],[animation,comedy,family,romance,musical]).
movie(the_shawshank_redemption,frank_darabont,r,142,[netflix],[drama]).
movie(the_lord_of_the_rings_the_fellowship_of_the_ring,peter_jackson,pg13,130,[hbomax],[drama,mystery,sci-fi]).
...
movie(spectre,sam_mendes,pg13,148,[hbomax],[action]).
movie(the_dark_knight,christopher_nolan,pg13,152,[hbomax],[action,drama,crime]).
